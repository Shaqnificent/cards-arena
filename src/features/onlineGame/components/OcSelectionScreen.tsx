...
export function OcSelectionScreen({ state, message, pending, onLock }: Props) {
  ...
  const selectOc = (characterId: string) => {
  ...
  return <main className="oc-selection-page"><header className="game-header"><span className="brand-link">ANIME ARENA</span><span>Secret OC Selection</span><span className="nav-link">Match {state.matchId.slice(0, 8)}</span></header><section className="oc-selection-content">
    ...
    <div className="oc-selection-layout"><section className="oc-selection-panel"><div className="oc-selection-panel-heading"><div><p className="eyebrow">Your OC Family</p><h2>{state.yourProfile.username} <SystemBadge visible={state.yourProfile.is_system_player} /></h2></div>{state.yourLocked && <span>Locked In</span>}</div>
    ...
    </section><section className="oc-selection-panel opponent"><div className="oc-selection-panel-heading"><div><p className="eyebrow">Opponent OC Family</p><h2>{state.opponentProfile.username} <SystemBadge visible={state.opponentProfile.is_system_player} /></h2></div>{state.opponentLocked && <span>Selection Locked</span>}</div>
