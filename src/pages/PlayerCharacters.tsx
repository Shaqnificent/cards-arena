...
export function PlayerCharacters({ username, avatarUrl }: PlayerCharactersProps) {
  ...
  return <main className="oc-page">
    ...
    {formOpen && <div className="oc-modal-backdrop" role="presentation"><section className="oc-modal" role="dialog" aria-modal="true" aria-labelledby="create-oc-heading">{created ? <div className="oc-reveal"><p className="eyebrow">Character Created</p><CharacterIdentity character={created} /><p>Your potential has been determined</p><div className="oc-reveal-stats"><div><span>Starting OVR</span><strong>{created.starting_overall}</strong></div><div><span>Growth Type</span><strong>{getGrowthType(created.starting_overall)}</strong></div><div><span>OVR Cap</span><strong>{created.overall_cap}</strong></div><div><span>Battle Power</span><strong>{formatPower(created.power_score)}</strong></div><div><span>Power Cap</span><strong>{formatPower(created.power_score_cap)}</strong></div></div><button className="button button-primary" onClick={() => setFormOpen(false)}>View My Fighter</button></div>
        : <><div className="oc-modal-heading"><div><p className="eyebrow">New Fighter</p><h2 id="create-oc-heading">Create OC</h2></div><button onClick={() => setFormOpen(false)} aria-label="Close creation form">×</button></div><label>OC Name<input maxLength={50} value={name} onChange={(event) => setName(event.target.value)} placeholder="Enter fighter name" /></label><label>Verse<select value={selectedVerseId} onChange={(event) => setVerseId(event.target.value)}><option value="" disabled>Select a verse</option>{verses.map((verse) => <option key={verse.id} value={verse.id}>{verse.name}</option>)}</select></label><fieldset className="oc-type-picker"><legend>Fighter Type</legend><button type="button" className={ocType === 'champion' ? 'selected' : ''} aria-pressed={ocType === 'champion'} onClick={() => setOcType('champion')}><strong>Champion</strong><span>Best for direct combat. Absorb a same-verse drafted fighter to raise your OC's temporary OVR.</span></button><button type="button" className={ocType === 'sacrificial' ? 'selected' : ''} aria-pressed={ocType === 'sacrificial'} onClick={() => setOcType('sacrificial')}><strong>Sacrificial</strong><span>Best for support. Give up this OC to increase the Battle Power of every drafted fighter from the same verse.</span></button><small>Your OC Type affects how the character can be used in matches. This choice is permanent.</small></fieldset>{actionError && <p className="error-message" role="alert">{actionError}</p>}<div className="oc-modal-actions"><button className="button button-secondary" onClick={() => setFormOpen(false)}>Cancel</button><button className="button button-primary" disabled={!ocType || collection.pendingId === 'create'} onClick={() => void submitCreation()}>{collection.pendingId === 'create' ? 'Evaluating Potential...' : 'Create Fighter'}</button></div></>}</section></div>}
