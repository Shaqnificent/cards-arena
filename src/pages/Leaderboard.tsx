...
function FamilyRows({rows,currentUserId,sort}:{rows:OcFamilyRank[];currentUserId:string;sort:OcLeaderboardSort}){return <div className="oc-rank-list">{rows.map(r=><article key={r.ownerId} className={`oc-ranking-card family-card rank-card-${r.rank<=3?r.rank:'standard'} ${r.ownerId===currentUserId?'current-player':''}`}><RankBadge rank={r.rank}/><PlayerAvatar compact username={r.username} avatarUrl={r.avatarUrl} avatarMode={r.avatarMode} avatarBgColor={r.avatarBgColor} avatarTextColor={r.avatarTextColor}/><div className="rank-identity"><h2>{r.username}{r.ownerId===currentUserId&&<small>You</small>}</h2><p>Active OC Family</p><span>3 fighters equipped</span></div><div className="family-averages"><span>{sort==='power'?<><small>Avg Power</small><strong>{Number(r.avgPowerScore).toLocaleString(undefined,{maximumFractionDigits:1})}</strong></>:sort==='growth'?<><small>Total Growth</small><strong>+{r.totalGrowth}</strong></>:<><small>Avg OVR</small><strong>{Number(r.avgOverall).toFixed(1)}</strong></>}</span></div></article>)}</div>}
