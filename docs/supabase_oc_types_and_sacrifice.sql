-- Anime Arena OC Types + Sacrificial Support (post Phase 6)
-- Run after the current OC foundation, selection, sacrifice, battle integration,
-- portrait, and leaderboard migrations. public.verses.id is BIGINT.

-- 1. Permanent OC roles. Existing rows remain Champions but receive one choice.
alter table public.player_characters
  add column if not exists oc_type text not null default 'champion',
  add column if not exists type_selected_at timestamptz;
alter table public.player_characters drop constraint if exists player_characters_oc_type_check;
alter table public.player_characters add constraint player_characters_oc_type_check
  check (oc_type in ('champion', 'sacrificial'));

create or replace function public.protect_player_character_type() returns trigger language plpgsql set search_path='' as $$
begin
  if old.type_selected_at is not null and (new.oc_type<>old.oc_type or new.type_selected_at is distinct from old.type_selected_at) then
    raise exception using errcode='23514',message='This OC type is permanent.';
  end if;
  return new;
end $$;
drop trigger if exists protect_player_character_type on public.player_characters;
create trigger protect_player_character_type before update of oc_type,type_selected_at on public.player_characters for each row execute function public.protect_player_character_type();

-- New creation is explicit and server authoritative. The actual verse key is BIGINT.
create or replace function public.create_player_character(p_name text, p_verse_id bigint, p_oc_type text)
...
begin
  ...
  if not exists(select 1 from public.profiles where id=caller_id) then raise exception using errcode='23503', message='Player profile not found.'; end if;
  ...
  elsif band_roll < .90 then rolled_overall:=56+floor(random()*3)::integer; derived_overall_cap:=94; derived_power_cap:=9000;
  else rolled_overall:=59+floor(random()*2)::integer; derived_overall_cap:=95; derived_power_cap:=8500; end if;
  insert into public.player_characters(owner_id,verse_id,name,starting_overall,overall,overall_cap,starting_power_score,power_score,power_score_cap,progression_points,oc_type,type_selected_at)
  values(caller_id,p_verse_id,normalized_name,rolled_overall,rolled_overall,derived_overall_cap,5000,5000,derived_power_cap,0,p_oc_type,now()) returning * into created_character;
  return created_character;
end $$;

-- 5. Canonical Phase 5 resolver, patched only to consume match snapshots and
-- Sacrificial Power rows and to reject support OCs as battle reserves.
...
create or replace function public.select_player_character_type(p_character_id uuid, p_oc_type text)
returns public.player_characters language plpgsql security definer set search_path='' as $$
declare caller_id uuid:=auth.uid(); chosen public.player_characters%rowtype;
begin
  if caller_id is null then raise exception using errcode='42501', message='Authentication required.'; end if;
  if p_oc_type not in ('champion','sacrificial') then raise exception using errcode='22023', message='Choose Champion or Sacrificial.'; end if;
  select * into chosen from public.player_characters where id=p_character_id and owner_id=caller_id for update;
  if not found then raise exception using errcode='42501', message='OC not found or not owned by you.'; end if;
  if chosen.type_selected_at is not null then raise exception using errcode='23514', message='This OC type is already permanent.'; end if;
  update public.player_characters set oc_type=p_oc_type,type_selected_at=now() where id=p_character_id returning * into chosen;
  return chosen;
end $$;

-- Browser roles retain no direct UPDATE privilege.
revoke all on function public.create_player_character(text,bigint,text), public.select_player_character_type(uuid,text) from public;
grant execute on function public.create_player_character(text,bigint,text), public.select_player_character_type(uuid,text) to authenticated;
do $$ begin
  if to_regprocedure('public.create_player_character(text,bigint)') is not null then
    execute 'revoke all on function public.create_player_character(text,bigint) from public, anon, authenticated';
  end if;
  if to_regprocedure('public.create_player_character(text,uuid)') is not null then
    execute 'revoke all on function public.create_player_character(text,uuid) from public, anon, authenticated';
  end if;
end $$;

-- 2. Match snapshots. Triggers preserve the canonical draft/OC initializers.
alter table public.match_oc_options add column if not exists oc_type_snapshot text;
update public.match_oc_options o set oc_type_snapshot=pc.oc_type from public.player_characters pc where pc.id=o.player_character_id and o.oc_type_snapshot is null;
alter table public.match_oc_options alter column oc_type_snapshot set default 'champion';
alter table public.match_oc_options alter column oc_type_snapshot set not null;
alter table public.match_oc_options drop constraint if exists match_oc_options_oc_type_check;
alter table public.match_oc_options add constraint match_oc_options_oc_type_check check(oc_type_snapshot in ('champion','sacrificial'));

alter table public.match_oc_selections add column if not exists oc_type_snapshot text;
update public.match_oc_selections s set oc_type_snapshot=o.oc_type_snapshot from public.match_oc_options o
where o.match_id=s.match_id and o.player_id=s.player_id and o.player_character_id=s.player_character_id and s.oc_type_snapshot is null;
alter table public.match_oc_selections drop constraint if exists match_oc_selections_oc_type_check;
alter table public.match_oc_selections add constraint match_oc_selections_oc_type_check check(oc_type_snapshot is null or oc_type_snapshot in ('champion','sacrificial'));

create or replace function public.snapshot_match_oc_role() returns trigger language plpgsql security definer set search_path='' as $$
begin
  select pc.oc_type into new.oc_type_snapshot from public.player_characters pc where pc.id=new.player_character_id;
  if new.oc_type_snapshot is null then raise exception using errcode='23514',message='OC role snapshot is unavailable.'; end if;
  return new;
end $$;
drop trigger if exists snapshot_match_oc_role on public.match_oc_options;
create trigger snapshot_match_oc_role before insert on public.match_oc_options for each row execute function public.snapshot_match_oc_role();

create or replace function public.snapshot_selected_oc_role() returns trigger language plpgsql security definer set search_path='' as $$
begin
  if new.player_character_id is not null then
    select o.oc_type_snapshot into new.oc_type_snapshot from public.match_oc_options o where o.match_id=new.match_id and o.player_id=new.player_id and o.player_character_id=new.player_character_id;
    if new.oc_type_snapshot is null then raise exception using errcode='23514',message='Selected OC role snapshot is unavailable.'; end if;
  end if;
  return new;
end $$;
drop trigger if exists snapshot_selected_oc_role on public.match_oc_selections;
create trigger snapshot_selected_oc_role before insert on public.match_oc_selections for each row execute function public.snapshot_selected_oc_role();

alter table public.match_characters
  add column if not exists overall_snapshot bigint,
  add column if not exists power_score_snapshot bigint,
  add column if not exists verse_id_snapshot bigint references public.verses(id) on delete restrict;
update public.match_characters mc set overall_snapshot=coalesce(mc.overall_snapshot,c.overall),power_score_snapshot=coalesce(mc.power_score_snapshot,c.power_score),verse_id_snapshot=coalesce(mc.verse_id_snapshot,c.verse_id)
from public.characters c where c.id=mc.character_id and (mc.overall_snapshot is null or mc.power_score_snapshot is null or mc.verse_id_snapshot is null);
alter table public.match_characters alter column overall_snapshot set not null;
alter table public.match_characters alter column power_score_snapshot set not null;
alter table public.match_characters alter column verse_id_snapshot set not null;

create or replace function public.snapshot_match_character_stats() returns trigger language plpgsql security definer set search_path='' as $$
begin
  if new.overall_snapshot is null or new.power_score_snapshot is null or new.verse_id_snapshot is null then
    select c.overall,c.power_score,c.verse_id into new.overall_snapshot,new.power_score_snapshot,new.verse_id_snapshot from public.characters c where c.id=new.character_id;
  end if;
  return new;
end $$;
drop trigger if exists snapshot_match_character_stats on public.match_characters;
create trigger snapshot_match_character_stats before insert on public.match_characters for each row execute function public.snapshot_match_character_stats();

-- 3. Centralized Sacrificial transfer math: floor(20%), absolute OVR gap,
-- max +2,000 per recipient, max 12,000 temporary Power.
create or replace function public.get_oc_power_compatibility_percent(p_difference integer)
returns integer language sql immutable set search_path='' as $$
  select case when abs(p_difference)<=4 then 100 when abs(p_difference)<=9 then 85 when abs(p_difference)<=14 then 70 when abs(p_difference)<=19 then 55 else 40 end;
$$;
create or replace function public.calculate_oc_power_transfer(p_oc_power integer,p_oc_overall integer,p_recipient_overall integer,p_recipient_power integer)
returns table(base_transfer integer,ovr_difference integer,compatibility_percent integer,calculated_bonus integer,effective_bonus integer,match_power_score integer)
language sql immutable set search_path='' as $$
  with x as(select floor(p_oc_power*20/100.0)::integer base,abs(p_recipient_overall-p_oc_overall)::integer gap),
  y as(select base,gap,public.get_oc_power_compatibility_percent(gap) pct from x),
  z as(select base,gap,pct,floor(base*pct/100.0)::integer calc from y)
  select base,gap,pct,calc,least(2000,calc),least(12000,p_recipient_power+least(2000,calc)) from z;
$$;
...
create table if not exists public.match_oc_power_boosts(
  ...
  oc_overall integer not null, oc_power_score integer not null, recipient_overall integer not null,
  recipient_base_power integer not null, ovr_difference integer not null, compatibility_percent integer not null,
  calculated_bonus integer not null, effective_bonus integer not null, match_power_score integer not null,
  created_at timestamptz not null default now(), primary key(match_id,match_character_id),
  check(compatibility_percent in(40,55,70,85,100)), check(effective_bonus between 0 and 2000), check(match_power_score between recipient_base_power and 12000)
);
create index if not exists match_oc_power_boosts_player_idx on public.match_oc_power_boosts(match_id,player_id);
...
do $$ begin
  if exists(select 1 from pg_publication_tables pt join pg_publication p on p.pubname=pt.pubname where pt.pubname='supabase_realtime' and pt.schemaname='public' and pt.tablename='match_oc_power_boosts' and not p.puballtables)
  then execute 'alter publication supabase_realtime drop table public.match_oc_power_boosts'; end if;
end $$;

-- 4. Extend the private preparation snapshot.
alter table public.match_oc_preparations
  add column if not exists oc_type text,
  add column if not exists oc_sacrificed boolean not null default false,
  add column if not exists base_transfer_power integer not null default 0,
  add column if not exists recipient_count integer not null default 0;
update public.match_oc_preparations p set oc_type=s.oc_type_snapshot from public.match_oc_selections s
where s.match_id=p.match_id and s.player_id=p.player_id and p.player_character_id is not null and p.oc_type is null;
update public.match_oc_preparations set decision='absorb' where decision='sacrifice' and sacrificed_match_character_id is not null;
do $$ declare c record; begin
  for c in select conname from pg_constraint where conrelid='public.match_oc_preparations'::regclass and contype='c'
...
alter table public.match_oc_preparations add constraint match_oc_preparations_role_check check(
...
alter table public.match_oc_preparations add constraint match_oc_preparations_role_values_check
  check(sacrifice_boost between 0 and 6 and base_transfer_power>=0 and recipient_count>=0);
-- The former "inactive" choice represented declining the special mode. Under
-- the canonical rules that is the default Reserve path for every OC Type.
...
-- Public family roles are visible, but the selected OC remains private.
...
begin
  ...
  return jsonb_build_object('matchId',match_row.id,'status',match_row.status,'yourPlayerId',caller_id,'opponentPlayerId',opponent_id,
...
begin
  ...
  return jsonb_build_object('matchId',p_match_id,'status',match_row.status,'yourPlayerId',caller_id,
    'yourOC',(select case when s.player_character_id is null then null else jsonb_build_object('characterId',s.player_character_id,'name',o.name_snapshot,'verseId',s.verse_id,'verseName',o.verse_name_snapshot,'baseOverall',s.base_overall,'powerScore',s.base_power_score,'ocType',s.oc_type_snapshot) end from public.match_oc_selections s left join public.match_oc_options o on o.match_id=s.match_id and o.player_id=s.player_id and o.player_character_id=s.player_character_id where s.match_id=p_match_id and s.player_id=caller_id),
    'eligibleSacrifices',coalesce((select jsonb_agg(jsonb_build_object('matchCharacterId',mc.id,'characterId',c.id,'name',c.name,'verseId',mc.verse_id_snapshot,'verseName',v.name,'overall',mc.overall_snapshot,'powerScore',mc.power_score_snapshot,'tier',public.get_character_tier(mc.overall_snapshot),'sacrificeBoost',public.get_sacrifice_ovr_boost(mc.overall_snapshot),'compatibilityPercent',x.compatibility_percent,'effectiveBonus',x.effective_bonus,'matchPowerScore',x.match_power_score) order by mc.draft_position)
      from public.match_characters mc join public.characters c on c.id=mc.character_id join public.verses v on v.id=mc.verse_id_snapshot
      join public.match_oc_selections s on s.match_id=mc.match_id and s.player_id=caller_id
      cross join lateral public.calculate_oc_power_transfer(s.base_power_score,s.base_overall,mc.overall_snapshot::integer,mc.power_score_snapshot::integer) x
      where mc.match_id=p_match_id and mc.owner_player_id=caller_id and s.player_character_id is not null and mc.verse_id_snapshot=s.verse_id),'[]'::jsonb),
...
begin
  ...
  if selection_row.oc_type_snapshot='champion' then
    if p_decision='reserve' then
      ...
      insert into public.match_oc_preparations(match_id,player_id,player_character_id,verse_id,oc_type,decision,base_overall,match_overall,base_power_score)
      values(p_match_id,caller_id,selection_row.player_character_id,selection_row.verse_id,'champion','reserve',selection_row.base_overall,selection_row.base_overall,selection_row.base_power_score);
      ...
      insert into public.match_oc_preparations(match_id,player_id,player_character_id,verse_id,oc_type,decision,sacrificed_match_character_id,sacrifice_tier,sacrifice_boost,base_overall,match_overall,base_power_score)
      values(p_match_id,caller_id,selection_row.player_character_id,selection_row.verse_id,'champion','absorb',card_row.id,tier_value,boost_value,selection_row.base_overall,least(99,selection_row.base_overall+boost_value),selection_row.base_power_score);
    ...
    if p_decision='reserve' then
      insert into public.match_oc_preparations(match_id,player_id,player_character_id,verse_id,oc_type,decision,base_overall,match_overall,base_power_score)
      values(p_match_id,caller_id,selection_row.player_character_id,selection_row.verse_id,'sacrificial','reserve',selection_row.base_overall,selection_row.base_overall,selection_row.base_power_score);
      ...
      insert into public.match_oc_preparations(match_id,player_id,player_character_id,verse_id,oc_type,decision,oc_sacrificed,base_overall,match_overall,base_power_score,base_transfer_power,recipient_count)
      values(p_match_id,caller_id,selection_row.player_character_id,selection_row.verse_id,'sacrificial','sacrifice',true,selection_row.base_overall,selection_row.base_overall,selection_row.base_power_score,transfer,recipients);
      ...
      select p_match_id,caller_id,selection_row.player_character_id,mc.id,selection_row.base_overall,selection_row.base_power_score,mc.overall_snapshot,mc.power_score_snapshot,x.ovr_difference,x.compatibility_percent,x.calculated_bonus,x.effective_bonus,x.match_power_score
      from public.match_characters mc cross join lateral public.calculate_oc_power_transfer(selection_row.base_power_score,selection_row.base_overall,mc.overall_snapshot::integer,mc.power_score_snapshot::integer) x
      where mc.match_id=p_match_id and mc.owner_player_id=caller_id and mc.verse_id_snapshot=selection_row.verse_id on conflict(match_id,match_character_id) do nothing;
...
begin
  ...
  return jsonb_build_object(
    ...
    'yourProfile',(select jsonb_build_object('id',p.id,'username',p.username,'wins',p.wins,'losses',p.losses) from public.profiles p where p.id=caller_id),
    'opponentProfile',(select jsonb_build_object('id',p.id,'username',p.username) from public.profiles p where p.id=opponent_id),
...
-- Private-state and function privileges.
revoke all on function public.protect_player_character_type(),public.snapshot_match_oc_role(),public.snapshot_selected_oc_role(),public.snapshot_match_character_stats() from public;
revoke all on function public.get_oc_power_compatibility_percent(integer),public.calculate_oc_power_transfer(integer,integer,integer,integer) from public;
revoke all on function public.get_match_oc_selection_state(uuid),public.get_match_oc_preparation_state(uuid),public.submit_match_oc_preparation(uuid,text,uuid) from public;
revoke all on function public.submit_battle_selection(uuid,text,uuid),public.submit_battle_selection(uuid,uuid),public.get_online_battle_state(uuid) from public;
grant execute on function public.get_match_oc_selection_state(uuid),public.get_match_oc_preparation_state(uuid),public.submit_match_oc_preparation(uuid,text,uuid) to authenticated;
grant execute on function public.submit_battle_selection(uuid,text,uuid),public.submit_battle_selection(uuid,uuid),public.get_online_battle_state(uuid) to authenticated;
