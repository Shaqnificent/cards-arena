...
begin
  ...
  return jsonb_build_object(
    ...
    'yourProfile', (select jsonb_build_object('id',p.id,'username',p.username,'avatar_url',p.avatar_url,'is_guest',p.is_guest,'is_admin',p.is_admin,'is_system_player',p.is_system_player,'wins',p.wins,'losses',p.losses,'created_at',p.created_at) from public.profiles p where p.id = caller_id),
    'opponentProfile', (select jsonb_build_object('id',p.id,'username',p.username,'avatar_url',p.avatar_url,'is_guest',p.is_guest,'is_admin',p.is_admin,'is_system_player',p.is_system_player,'wins',p.wins,'losses',p.losses,'created_at',p.created_at) from public.profiles p where p.id = opponent_id),
